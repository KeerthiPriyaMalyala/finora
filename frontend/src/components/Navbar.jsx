...
export function Navbar() {
  ...
  return (
    <header
      className="h-16 flex items-center px-4 sm:px-6 relative overflow-hidden z-10"
      style={{
    ...
    >
      ...
      {/* Ambient glow */}
      <div className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 w-64 h-16 rounded-full bg-teal-500/10 blur-2xl" />
      ...
      >
        ...
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#062024" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="22 7 13.5 15.5 8.5 10.5 2 17" />
            <polyline points="16 7 22 7 22 13" />
          </svg>
        </div>
        ...
        <span className="nav-dot w-1.5 h-1.5 rounded-full bg-teal-400 ml-0.5" style={{ boxShadow: "0 0 6px rgba(45,212,191,0.8)" }} />
      </div>

      {/* Right side */}
      <div className="flex items-center gap-2 sm:gap-3 relative ml-auto">
        {/* User pill */}
        <div
          className="flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-1.5 rounded-xl"
          style={{
        ...
        >
          <div
            className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
            style={{
              background: "linear-gradient(135deg, rgba(45,212,191,0.3), rgba(245,158,11,0.2))",
        ...
        <button
          onClick={logout}
          className="logout-btn flex items-center gap-1.5 text-sm font-medium text-white/35 px-2.5 sm:px-3 py-1.5 rounded-xl border border-transparent"
        >
          <LogOut size={14} />
          <span className="hidden sm:inline">Logout</span>
        </button>
