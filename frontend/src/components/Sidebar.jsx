...
export function Sidebar() {
  // "menu"   -> mobile shows the full-screen nav menu
  // "hidden" -> mobile shows the page content + a small back-to-menu button
    ...
    <nav className="flex flex-col gap-1 relative">
      {links.map(({ to, label, icon: Icon }, i) => (
        ...
        >
          {({ isActive }) => (
            <span
              className="sidebar-link flex items-center gap-3 px-3 py-3 lg:py-2.5 rounded-xl text-sm font-medium relative overflow-hidden"
              style={
                isActive
                  ? {
                      background: "linear-gradient(135deg, rgba(45,212,191,0.22), rgba(245,158,11,0.12))",
                      border: "1px solid rgba(45,212,191,0.35)",
                      color: "#5eead4",
                      boxShadow: "0 0 16px rgba(45,212,191,0.18), inset 0 0 16px rgba(45,212,191,0.05)",
                    }
                  : {
                      color: "rgba(255,255,255,0.35)",
                      border: "1px solid transparent",
  ...
  return (
    <>
      <style>{`
        ...
        .glow-orb { animation: glowPulse 3s ease-in-out infinite; }
        .nav-item { animation: slideIn 0.35s ease both; }
        .mobile-menu-fade { animation: fadeIn 0.25s ease both; }
      `}</style>

      {/* ============ MOBILE: full-screen menu (covers the page) ============ */}
      <div
        className={`lg:hidden ${mobileView === "menu" ? "flex" : "hidden"} mobile-menu-fade fixed inset-0 z-50 flex-col py-6 px-4`}
        style={{ background: "linear-gradient(180deg, #080B14 0%, #0c1420 60%, #080B14 100%)" }}
      >
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="glow-orb absolute -top-10 -left-10 w-40 h-40 rounded-full bg-teal-500/15 blur-3xl" />
          <div className="glow-orb absolute bottom-20 -right-6 w-32 h-32 rounded-full bg-amber-500/12 blur-2xl" style={{ animationDelay: "1.5s" }} />
        ...
        <div className="relative mb-8 px-2">
          <Logo />
          <p className="text-[11px] text-white/25 pl-[38px] font-medium tracking-widest uppercase mt-1">
            Finance Manager
          </p>
        </div>

        <p className="relative text-[10px] font-bold text-white/20 uppercase tracking-[0.15em] px-2 mb-2">
          Navigation
        </p>

        <div className="relative">
          <NavList onLinkClick={() => setMobileView("hidden")} />
        </div>
      </div>

      {/* ============ MOBILE: floating back-to-menu button (shown on individual pages) ============ */}
      <button
        onClick={() => setMobileView("menu")}
        className={`lg:hidden ${mobileView === "hidden" ? "flex" : "hidden"} fixed top-4 left-4 z-40 items-center gap-1.5 pl-2.5 pr-3.5 py-2 rounded-full text-xs font-medium text-teal-200`}
        style={{
          background: "rgba(8,11,20,0.85)",
      ...
      {/* ============ DESKTOP: classic always-visible sidebar ============ */}
      <aside
        className="hidden lg:flex w-56 shrink-0 min-h-screen flex-col py-6 px-3 relative overflow-hidden"
        style={{
          background: "linear-gradient(180deg, #080B14 0%, #0c1420 60%, #080B14 100%)",
          borderRight: "1px solid rgba(45,212,191,0.12)",
        }}
      >
        ...
        <p className="relative text-[10px] font-bold text-white/20 uppercase tracking-[0.15em] px-3 mb-2">
          Navigation
        </p>
