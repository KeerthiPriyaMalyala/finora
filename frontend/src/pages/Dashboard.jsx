...
// const COLORS = ["#2dd4bf", "#f59e0b", "#38bdf8", "#fb7185", "#a3e635", "#c084fc", "#fbbf24", "#94a3b8"];

// const PAGE_BG = "linear-gradient(135deg, #080B14 0%, #0c1420 50%, #080B14 100%)";

// const glassCard =
//   "relative backdrop-blur-2xl bg-white/[0.04] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden";

// const CATEGORY_COLORS = {
//   Food: "#fb923c", Travel: "#60a5fa", Bills: "#c084fc", Shopping: "#f472b6",
//   Entertainment: "#fbbf24", Health: "#f87171", Education: "#22d3ee",
//   Salary: "#4ade80", Investment: "#2dd4bf", Others: "#94a3b8",
// };
...
// function SectionHeader({ color = "#5eead4", dotColor = "#2dd4bf", children }) {
//   return (
//     <div className="flex items-center gap-2 mb-5">
//       <span
//         className="w-2 h-2 rounded-full"
...
//       style={{
//         background:
//           "linear-gradient(90deg,rgba(45,212,191,0.06) 0%,rgba(245,158,11,0.12) 50%,rgba(45,212,191,0.06) 100%)",
//         backgroundSize: "200% 100%",
//         animation: "shimmer 1.8s infinite",
//       }}
//     />
...
//   // Animated count-up values for the stat cards (purely visual, same source data)
//   const animatedIncome = useCountUp(totalIncome);
//   const animatedExpense = useCountUp(totalExpense);
//   const animatedNetBalance = useCountUp(totalIncome - totalExpense);
//   const animatedSavingsPct = useCountUp(Number(savingsPct));
...
//   if (error) {
//     return (
//       <div className="relative p-4 sm:p-6 min-h-screen" style={{ background: PAGE_BG }}>
//         <div className="backdrop-blur-xl bg-red-500/10 border border-red-500/25 rounded-2xl p-4 text-red-400 text-sm">
//           {error}
//         </div>
//       </div>
...
//       {/* Orbs */}
//       <div className="pointer-events-none absolute inset-0 overflow-hidden">
//         <div className="absolute -top-24 -left-24 w-[300px] h-[300px] sm:w-[480px] sm:h-[480px] rounded-full bg-teal-500/14 blur-3xl" />
//         <div className="absolute top-1/3 -right-20 w-56 h-56 sm:w-80 sm:h-80 rounded-full bg-amber-500/10 blur-3xl" />
//         <div className="absolute bottom-0 left-1/3 w-52 h-52 sm:w-72 sm:h-72 rounded-full bg-teal-400/8 blur-3xl" />
//       </div>

//       {/* Page header */}
//       <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 fadein-0">
//         <div className="flex items-center gap-3">
...
//           style={{ background: "rgba(45,212,191,0.1)", border: "1px solid rgba(45,212,191,0.2)", color: "rgba(94,234,212,0.85)" }}
//         >
//           <Calendar size={12} />
//           {new Date().toLocaleDateString("en-IN", { month: "long", year: "numeric" })}
//         </div>
...
//       {/* Stat cards */}
//       <div className="relative grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 fadein-1">
//         <StatCard title="Total Income" value={fmt(Math.round(animatedIncome))} icon={TrendingUp} color="green" />
//         <StatCard title="Total Expense" value={fmt(Math.round(animatedExpense))} icon={TrendingDown} color="red" />
//         <StatCard title="Net Balance" value={fmt(Math.round(animatedNetBalance))} icon={Wallet} color="indigo" />
//         <StatCard title="Savings Rate" value={`${animatedSavingsPct.toFixed(1)}%`} icon={PiggyBank} color="yellow" subtitle="of income saved" />
//       </div>
...
//             <span className="text-xs font-semibold text-white/50 uppercase tracking-widest">Savings Progress</span>
//             <span className="text-xs font-bold" style={{ color: "#5eead4" }}>{savingsPct}% of income saved</span>
//           </div>
//           <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
//             <div
//               className="h-full rounded-full transition-all duration-1000"
...
//       {/* Charts row */}
//       <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 fadein-2">
//         {/* Pie chart */}
//         <div className={glassCard}>
...
//             <SectionHeader dotColor="#2dd4bf">Expense Breakdown</SectionHeader>
//             {expenseByCategory.length ? (
//               <>
//                 <ResponsiveContainer width="100%" height={200}>
//                   <PieChart>
...
//                     <Tooltip
//                       formatter={(v) => fmt(v)}
//                       contentStyle={{ background: "rgba(8,11,20,0.97)", border: "1px solid rgba(45,212,191,0.3)", borderRadius: "12px" }}
...
//       {/* Income vs Expense Bar chart */}
//       <div className={`relative ${glassCard} fadein-3`}>
//         <div className="absolute inset-0 bg-gradient-to-br from-amber-600/5 to-teal-600/5 pointer-events-none" />
//         <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-amber-400/40 to-transparent" />
//         <div className="relative p-5 sm:p-6">
//           <SectionHeader dotColor="#f59e0b">Income vs Expense — Bar View</SectionHeader>
//           {monthlyTrend.length ? (
//             <ResponsiveContainer width="100%" height={200}>
//               <BarChart data={monthlyTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
//                 <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
...
//       <div className={`relative ${glassCard} fadein-4`}>
//         <div className="absolute inset-0 bg-gradient-to-br from-teal-500/5 to-amber-500/5 pointer-events-none" />
//         <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-teal-400/35 to-transparent" />
//         <div className="relative p-5 sm:p-6">
//           <SectionHeader dotColor="#2dd4bf">Recent Transactions</SectionHeader>
...
//                   <div
//                     key={tx._id}
//                     className="tx-row flex items-center gap-3 px-2 sm:px-3 py-3 rounded-xl transition-colors duration-150"
//                   >
//                     {/* Category dot */}
//                     <div
//                       className="w-9 h-9 rounded-xl shrink-0 flex items-center justify-center text-xs font-bold"
...
//               >
//                 <Wallet size={22} className="text-teal-400/50" />
//               </div>
//               <div className="text-center">
//                 <p className="text-sm font-medium text-white/30">No recent transactions</p>
//                 <p className="text-xs text-white/18 mt-1">Add transactions to see them here</p>
...
export default function Dashboard() {
  ...
  return (
    <div className="relative p-4 sm:p-6 space-y-5 sm:space-y-6 min-h-screen" style={{ background: PAGE_BG }}>
      ...
      {/* Stat cards */}
      <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 fadein-1">
        <StatCard title="Total Income" value={fmt(Math.round(animatedIncome))} icon={TrendingUp} color="green" />
        <StatCard title="Total Expense" value={fmt(Math.round(animatedExpense))} icon={TrendingDown} color="red" />
        <StatCard title="Net Balance" value={fmt(Math.round(animatedNetBalance))} icon={Wallet} color="indigo" />
        <StatCard title="Savings Rate" value={`${animatedSavingsPct.toFixed(1)}%`} icon={PiggyBank} color="yellow" subtitle="of income saved" />
