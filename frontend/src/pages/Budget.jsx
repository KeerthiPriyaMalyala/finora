...
const glassCard = "relative backdrop-blur-2xl bg-white/[0.04] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden";
const inputCls  = "w-full px-4 py-2.5 rounded-xl bg-white/[0.05] border border-white/[0.1] text-white/90 placeholder-white/30 text-sm focus:outline-none focus:border-teal-400/60 focus:bg-white/[0.08] transition-all duration-200";
const selectCls = "w-full px-4 py-2.5 rounded-xl bg-slate-900/80 border border-white/[0.1] text-white/80 text-sm focus:outline-none focus:border-teal-400/60 transition-all duration-200";
const labelCls  = "block text-[11px] font-semibold text-white/40 uppercase tracking-widest mb-1.5";
...
function MiniStat({ label, value, color, icon: Icon }) {
  return (
    <div
      className="flex-1 min-w-[140px] rounded-xl p-4 flex items-center gap-3 min-w-0"
      style={{ background: `${color}0f`, border: `1px solid ${color}25` }}
    >
      <div
        className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
  ...
  return (
    <div className="space-y-2">
      {overBudget.map((b) => (
        <div
          key={b._id}
          className="flex items-start sm:items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium"
          style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", color: "#f87171" }}
        >
  ...
  return (
    <div className={`relative ${glassCard}`}>
      ...
      <div className="relative p-5 sm:p-6">
        <div className="flex items-center gap-2 mb-5">
          <span className="w-2 h-2 rounded-full bg-teal-400" style={{ boxShadow: "0 0 6px #2dd4bf" }} />
          <h3 className="text-[11px] font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.55)" }}>
            Category Spending Comparison
          </h3>
        </div>
...
export default function Budget() {
  ...
  return (
    <div className="relative p-4 sm:p-6 space-y-5 sm:space-y-6 min-h-screen" style={{ background: PAGE_BG }}>
      <style>{`
        @keyframes shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
        @keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
        @keyframes glowPulse{0%,100%{opacity:.45}50%{opacity:1}}
        ...
        .fadein-4{animation:fadeUp .45s .32s ease both}
        .glow-dot{animation:glowPulse 2.8s ease-in-out infinite}
        .submit-btn:hover:not(:disabled){box-shadow:0 0 28px rgba(45,212,191,0.5);transform:translateY(-1px)}
        .submit-btn:active:not(:disabled){transform:translateY(0)}
        .submit-btn{transition:all .2s ease}
        .filter-sel:hover{border-color:rgba(45,212,191,0.5)}
        .loading-ring{animation:spin360 .8s linear infinite}
      `}</style>

      {/* Orbs */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-16 left-1/4 w-[300px] h-[300px] sm:w-[480px] sm:h-[480px] rounded-full bg-teal-500/12 blur-3xl" />
        <div className="absolute top-2/3 -right-20 w-56 h-56 sm:w-80 sm:h-80 rounded-full bg-amber-500/10 blur-3xl" />
        <div className="absolute -bottom-10 left-10 w-52 h-52 sm:w-72 sm:h-72 rounded-full bg-teal-400/8 blur-3xl" />
      </div>

      {/* Header */}
      <div className="relative flex flex-wrap items-center gap-3 fadein-0">
        <div className="w-1 h-7 rounded-full bg-gradient-to-b from-teal-400 to-amber-400" style={{ boxShadow: "0 0 12px rgba(45,212,191,0.6)" }} />
        <h2
        ...
        <span className="glow-dot w-2 h-2 rounded-full bg-teal-400" style={{ boxShadow: "0 0 8px rgba(45,212,191,0.8)" }} />
        {budgets.length > 0 && (
          <span
            className="sm:ml-auto text-xs font-bold px-2.5 py-1 rounded-full"
      ...
      {/* ── Summary stats bar ── */}
      {!loading && budgets.length > 0 && (
        <div className="relative grid grid-cols-1 sm:grid-cols-3 gap-3 fadein-0">
          <MiniStat label="Total Budgeted"  value={fmt(totalBudgeted)}  color="#2dd4bf" icon={Target}    />
          <MiniStat label="Total Spent"     value={fmt(totalSpent)}     color="#ef4444" icon={Wallet}    />
          <MiniStat label="Total Remaining" value={fmt(totalRemaining)} color="#22c55e" icon={TrendingUp} />
        </div>
      )}
            ...
            <div
              ...
              style={{
                ...
                background: overallPct >= 100
                  ...
                  : overallPct >= 80
                  ? "linear-gradient(90deg,#f59e0b,#fbbf24)"
                  : "linear-gradient(90deg,#2dd4bf,#f59e0b)",
                boxShadow: overallPct >= 100 ? "0 0 10px rgba(239,68,68,0.5)" : overallPct >= 80 ? "0 0 10px rgba(245,158,11,0.5)" : "0 0 10px rgba(45,212,191,0.5)",
              }}
            />
          </div>
          <div className="flex flex-col sm:flex-row sm:justify-between mt-1.5 gap-0.5">
            <span className="text-[11px]" style={{ color: "rgba(255,255,255,0.28)" }}>{fmt(totalSpent)} spent</span>
            <span className="text-[11px]" style={{ color: "rgba(255,255,255,0.28)" }}>{fmt(totalBudgeted)} total limit</span>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex items-end">
              <button
                type="submit"
                disabled={submitting}
                className="submit-btn w-full py-2.5 rounded-xl text-sm font-bold text-[#062024] tracking-wide disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                style={{ background: "linear-gradient(135deg,#2dd4bf,#f59e0b)", boxShadow: "0 4px 20px rgba(45,212,191,0.3)" }}
              >
                {submitting ? (
                  <>
                    <svg className="loading-ring w-4 h-4" viewBox="0 0 24 24" fill="none">
                      <circle cx="12" cy="12" r="10" stroke="rgba(6,32,36,0.3)" strokeWidth="3" />
                      <path d="M12 2a10 10 0 0 1 10 10" stroke="#062024" strokeWidth="3" strokeLinecap="round" />
                    </svg>
                    Adding...
                  </>
                ) : (<><Plus size={15} /> Add Budget</>)}
      ...
      {/* ── Budget cards grid ── */}
      <div className={`relative ${glassCard} fadein-4`}>
        <div className="absolute inset-0 bg-gradient-to-br from-amber-500/5 to-teal-500/5 pointer-events-none" />
        ...
        <div className="relative p-5 sm:p-6">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <div className="flex items-center gap-2.5 flex-1 min-w-[140px]">
              <span className="w-2 h-2 rounded-full bg-amber-400 glow-dot" style={{ boxShadow: "0 0 6px #f59e0b" }} />
              <h3
                className="text-sm font-bold uppercase tracking-widest"
                style={{ background: "linear-gradient(90deg,#5eead4,#fbbf24)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}
              >
            ...
            <select value={filterMonth} onChange={(e) => setFilterMonth(Number(e.target.value))}
              className="filter-sel px-3 py-2 rounded-xl bg-slate-900/80 border border-white/[0.1] text-white/70 text-sm focus:outline-none focus:border-teal-400/60 transition-all">
              {months.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
            </select>
            <select value={filterYear} onChange={(e) => setFilterYear(Number(e.target.value))}
              className="filter-sel px-3 py-2 rounded-xl bg-slate-900/80 border border-white/[0.1] text-white/70 text-sm focus:outline-none focus:border-teal-400/60 transition-all">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-40 rounded-2xl"
                  style={{
                    background: "linear-gradient(90deg,rgba(45,212,191,0.06) 0%,rgba(245,158,11,0.12) 50%,rgba(45,212,191,0.06) 100%)",
                    backgroundSize: "200% 100%",
                    animation: `shimmer ${1.5 + i * 0.15}s ease-in-out infinite`,
                  }}
                />
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              <div
                className="w-16 h-16 rounded-2xl flex items-center justify-center"
                style={{ background: "rgba(45,212,191,0.1)", border: "1px solid rgba(45,212,191,0.2)", boxShadow: "0 0 24px rgba(45,212,191,0.1)" }}
              >
                <PiggyBank size={26} className="text-teal-400/60" />
              </div>
              <div className="text-center">
                <p className="text-sm font-medium text-white/30">No budgets set</p>
                <p className="text-xs text-white/18 mt-1">
